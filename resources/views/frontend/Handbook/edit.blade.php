            ...
            <form id="editForm" method="POST" action="{{ route('handbook.update', $handbook->id) }}"
                ...
                <div>
                    ...
                    <select id="category" name="category"
                        ...
                        <option value="SOP" {{ old('category', $handbook->category) == 'SOP' ? 'selected' : '' }}>SOP
                        </option>
                        <option value="Panduan Infrastruktur"
                            {{ old('category', $handbook->category) == 'Panduan Infrastruktur' ? 'selected' : '' }}>
                            Panduan Infrastruktur</option>
                        <option value="Prosedur Backup"
                            {{ old('category', $handbook->category) == 'Prosedur Backup' ? 'selected' : '' }}>Prosedur
                            Backup</option>
                        <option value="Kontak Darurat dan Eskalasi"
                            {{ old('category', $handbook->category) == 'Kontak Darurat dan Eskalasi' ? 'selected' : '' }}>
                            Kontak Darurat dan Eskalasi</option>
                        <option value="Checklist"
                            {{ old('category', $handbook->category) == 'Checklist' ? 'selected' : '' }}>Checklist
                        </option>
                    </select>
                </div>
                ...
                <div>
                    ...
                    <textarea id="description" name="description" rows="6"
                        class="block w-full rounded-md border-gray-300 shadow-sm focus:ring-emerald-500 focus:border-emerald-500" required>{{ old('description', $handbook->description) }}</textarea>
                </div>
